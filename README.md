...
- **Nén ảnh ở phía trình duyệt trước khi upload (`src/lib/compressImage.ts`)**: Vercel Serverless Functions từ chối thẳng mọi request có body > 4.5MB ở tầng platform (trả về lỗi không phải JSON, khiến `res.json()` phía client crash và hiện nhầm thành "Không thể kết nối máy chủ") — trong khi ảnh chụp thẳng từ camera điện thoại thường 3–10MB. Vì vậy trước khi gọi `/api/analyze`, ảnh được resize (tối đa 1600px cạnh dài, giữ tỉ lệ) và mã hoá lại JPEG chất lượng 0.82 ngay trong trình duyệt bằng Canvas API; ảnh đã nhỏ hơn 1.5MB thì giữ nguyên. `MAX_FILE_SIZE` phía server (`/api/analyze`) cũng hạ xuống 4MB làm lớp bảo vệ thứ hai, và toàn bộ handler được bọc try/catch để luôn trả JSON hợp lệ thay vì để lộ lỗi nền tảng khó đoán ra ngoài.
